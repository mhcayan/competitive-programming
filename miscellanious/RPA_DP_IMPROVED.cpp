...
int main()
{
    ...
    freopen("output_improved2.txt","w",stdout);
   ...
   /* 
    ...
    //printf("For Each Connection,Enter [node_u node_v cost]\n"); 
    //srand((unsigned) time(&t));
    while(e--)
    {
...
int Replica_Placement(int v,int k,int l,int c)
{
   ...
   //    printf("Call %d %d %d %d\n",v,k,l,c);
    if(v==1)
    {
      ...
      while(beg<end)
      {
       mid=(beg+end)/2;
       if(beg+end%2) mid++;
       r=Replica_Placement(adj[v][0],mid-1,1,cst[v][0])+Replica_Placement(adj[v][1],k-mid,1,cst[v][1]);
       L=Replica_Placement(adj[v][0],mid,1,cst[v][0])+Replica_Placement(adj[v][1],k-mid-1,1,cst[v][1]);
 ...
 /*      printf("%d %d %d\n",beg,end,mid);
       ...
       rs=Replica_Placement(adj[v][0],end,1,cst[v][0])+Replica_Placement(adj[v][1],k-end-1,1,cst[v][1]);
       return rs;
    }
    if(adj[v].size()==0)
    {
    ...
    if(k&&v<=S)
    {
      ...
      while(beg<end)
      {
       ...
       r=Replica_Placement(adj[v][0],mid-1,1,cst[v][0])+Replica_Placement(adj[v][1],k-mid,1,cst[v][1]);
       L=Replica_Placement(adj[v][0],mid,1,cst[v][0])+Replica_Placement(adj[v][1],k-mid-1,1,cst[v][1]);
    /*   printf("At node %d with replica %d cost %d,%d %d %d\n",v,k,c,beg,end,mid);
       printf(" %d %d %d=%d+%d\n",mid-1,k-mid,r,h1,h2);
       printf(" %d %d %d\n",mid,k-mid-1,L);*/
       if(r>L) beg=mid;
       else end=mid-1;
      ...
      while(beg<end)
      {
       mid=(beg+end)/2;
       if(beg+end%2) mid++;
       r=Replica_Placement(adj[v][0],mid-1,l+1,c+cst[v][0])+Replica_Placement(adj[v][1],k-mid+1,l+1,c+cst[v][1]);
       L=Replica_Placement(adj[v][0],mid,l+1,c+cst[v][0])+Replica_Placement(adj[v][1],k-mid,l+1,c+cst[v][1]);
       if(r>L) beg=mid;
       else end=mid-1;
      }
   L=Read[v]*c+Replica_Placement(adj[v][0],end,l+1,c+cst[v][0])+Replica_Placement(adj[v][1],k-end,l+1,c+cst[v][1]);
   return rs=MIN(rs,L);   
}
